...
const Utils = {
  ...
  parseReplyExpression(str: string) {
    const [topic, ...responsePayload] = str.split(" ") ?? [];
    const payload = new Function("return " + responsePayload.join(" "))();
...
async function main() {
  ...
  for (const line of definitions.trim().split("\n")) {
    ...
    const fn = async (context: string | Record<string, unknown>) => {
      ...
      return () => {
        try {
          const [replyTopic, replyPayload] = Utils.parseReplyExpression(replyExpression);
